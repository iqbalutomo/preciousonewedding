...
var $table = document.getElementById("data-promo"),
...
function loadFormEdit(id){
    $.ajax({
        type     : 'POST',
        url      : '<?=site_url()?>/admin/get_promo/'+id,
        dataType : 'json',
        data     : ({id:id}),
        success : function(data){
            ...
            for(i=0;i<data.length;i++){
                html = '<div class="input-field">'+
                            ...
                            '<input id="promo_title" name="nama_promo" type="text" class="validate" value="'+data[i].nama_promo+'" required="" aria-required="true">'+
                        '</div>'+
...
//get data promo for delete
function loadDeleteForm(id){
    $.ajax({
        type     : 'POST',
        url      : '<?=site_url()?>/admin/get_promo/'+id, //checkpoint
        dataType : 'json',
        data     : ({id:id}),
        success : function(data){
            console.log(data);
            ...
            var i;
            for(i=0;i<data.length;i++){
                id = '<a href="<?=site_url('admin/delete_promo/');?>'+data[i].id+'" class="waves-effect waves-light btn-small modal-trigger red"><i class="material-icons left">clear</i>Delete</a>';
            }
